...
inline void Annotator::annotate(){
...
std::vector<Stat> Annotator::getGradeResults()
{
	auto indexE1 = static_cast<typename std::underlying_type<utils::Grade>::type>(utils::Grade::E1);
	std::vector<Stat> results(indexE1+1);
	float perc = 1/data.size(); 
    for(auto& pair : group_data){
		...
		{
			results[indexE1].cnt += num;
			results[indexE1].perc += num*perc;
		}
		else{
			auto index = static_cast<typename std::underlying_type<utils::Grade>::type>(grade);
			results[index].cnt += num;
			results[index].perc += num*perc;
		}
	}
